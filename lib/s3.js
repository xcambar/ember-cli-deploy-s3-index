...
function hasMatchingRevision(revision) {
  return function(revisions) {
    var found = revisions.map(function(element) { return element.revision; }).indexOf(revision);
    if (found >= 0) {
      return RSVP.resolve();
    } else {
...
module.exports = CoreObject.extend({
  ...
  activate: function(options) {
    ...
    var filePatterns          = [].concat(options.filePattern);

    var files = filePatterns.map(function(filePattern) {
      ...
      return { revisions: that.fetchRevisions(options), params: params, revisionKey: revisionKey, indexKey: indexKey };
    })
    .map(function(fileInfo) {
      return fileInfo.revisions
      .then(hasMatchingRevision(options.revisionKey))
      .then(function() {
        return copyObject(fileInfo.params).then(function() {
          plugin.log('✔  ' + fileInfo.revisionKey + " => " + fileInfo.indexKey);
          return fileInfo;
        });
      })
    });

    return RSVP.all(files);
  },
